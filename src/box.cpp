...
bool Box::isSqueezed(const VolPart& vol) const
{
    ...
    if (vol.getDir()[0] != 0)
    {
        const VolPart& nVol = getVolPart(vol.getCoords()[0] + vol.getDir()[0], vol.getCoords()[1], vol.getCoords()[2]);
    ...
    if (vol.getDir()[1] != 0)
    {
        const VolPart& nVol = getVolPart(vol.getCoords()[0], vol.getCoords()[1] + vol.getDir()[1], vol.getCoords()[2]);
        if (nVol.type() == VolPart::VolType::Full)
        {
            n++;
        }
    ...
    if (vol.getDir()[2] != 0)
    {
        const VolPart& nVol = getVolPart(vol.getCoords()[0], vol.getCoords()[1], vol.getCoords()[2] + vol.getDir()[2]);
        if (nVol.type() == VolPart::VolType::Full)
        {
            n++;
        }
