...
{
    return xyz == vol.xyz && fillInfo == vol.fillInfo && ( fillInfo != VolType::Angle || dir == vol.dir);
}
...
VolPart& VolPart::operator+=(const VolPart& v)
{
    ...
    if (fillInfo == VolType::Angle && v.fillInfo == VolType::Angle && dir == -v.dir)
    {
...
bool VolPart::couldPlace(const VolPart& another) const
{
    ...
    return fillInfo == VolType::Empty
           || fillInfo == VolType::Angle && another.fillInfo == VolType::Angle && dir == -another.dir
...
bool VolPart::shareOneOfSides(const VolPart& another) const
{
    ...
    if (fillInfo == VolType::Empty || another.fillInfo == VolType::Empty) return false;
    if (fillInfo == VolType::Full && another.fillInfo == VolType::Full) return true;
