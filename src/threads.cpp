...
Thread::Thread(std::auto_ptr<Task> r) : task(r) {
  if(!task.get())
    printError(L"Thread(std::auto_ptr<Task> r) failed at ",
    __FILE__, __LINE__);
  hThread = 
...
unsigned WINAPI Thread::startThreadRunnable(LPVOID pVoid) {
  ...
  runnableThread->result = runnableThread->task->run();
...
Thread::~Thread() {
  if(wThreadID != GetCurrentThreadId()) {
    ...
    if(!rc) printError
      (L"CloseHandle failed at ",__FILE__, __LINE__);
  }
}
