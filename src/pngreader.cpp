...
int reNpot(int w) {
    ...
    // эту переменную конечно надо определять один раз при старте проги с
    // помощью String s = gl.glGetString(GL10.GL_EXTENSIONS);
    // NON_POWER_OF_TWO_SUPPORTED = s.contains("texture_2D_limited_npot") ||
    // s.contains("texture_npot") || s.contains("texture_non_power_of_two");
...
image readPng(const char* data, unsigned int length, bool gl_arrange) {
    image im;
    // пропускаем заголовок, хотя именно сюда можно добавить проверку PNG это
    // или JPEG, чтобы ф-ция сама определяла как грузить картинку
    ...
    png_get_IHDR(png_ptr, info_ptr, &im.imWidth, &im.imHeight, &im.bit_depth,
                 &im.color_type, nullptr, nullptr, nullptr);
    ...
    // если картинка содержит прозрачность то на каждый пиксель 4 байта (RGBA),
    // иначе 3 (RGB)
