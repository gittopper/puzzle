...
void VolPartRenderer::drawSquare(const Geometry::Vector& shift, const Geometry::Vector& v1,
                const Geometry::Vector& v2, const Geometry::Vector& v3,
                const Geometry::Vector& v4, const Geometry::Vector& n) const
{
    ...
    std::vector<Geometry::Vector> colors = {cur_color_, cur_color_, cur_color_, cur_color_};
    renderer_->render(reinterpret_cast<GLfloat*>(vertices.data()), reinterpret_cast<GLfloat*>(normals.data()),
                    reinterpret_cast<GLfloat*>(colors.data()), reinterpret_cast<GLushort*>(indices.data()), 6);
}

void VolPartRenderer::drawTriangle(const Geometry::Vector& shift, const Geometry::Vector& v1,
                  const Geometry::Vector& v2, const Geometry::Vector& v3,
